...
def calculate_position(row,column,hallway):
    global WIDTH, LENGTH
    object_position = []
    # now_chair = json.loads(request.get_method('auditorium', 'chair', '', ''))
    # now_chair = chair(now_chair["id"],float(now_chair["width"]),float(now_chair["length"]),float(now_chair["height"]))
    now_table = table('Step001', 0.5, 0.5,0.2)#自生成讲台台阶
    # now_lectern = json.loads(request.get_method('auditorium', 'desk', '', ''))
    # now_lectern = lectern(now_lectern["id"],float(now_lectern["width"]),float(now_lectern["length"]),float(now_lectern["height"]))
    now_chair=chair('Chair022',1.0,1.0,1.5)
    now_lectern=lectern('Desk028',1.2,0.4,1.1)
    ...
    # 房间坐标的生成
    length=(object_position[0].zPosition+FRONT_WALL+LENGTH/2)*2
    ...
    now_table.width = FRONT_WALL * 0.5
    z = float(format(length/2-now_table.width/2, '.3f'))
    position1 = position(now_table.id, 'step', 0.0, 0.0, z, float(format(now_table.length,'.3f')), float(format(now_table.width,'.3f')), 0.2)
    object_position.append(position1)
    # 演讲桌坐标的生成
    z = float(format((z - now_table.width / 2 + now_lectern.length / 2), '.3f'))
    ...
    LayoutObjects["dataForModify"] = {"type":"auditorium"}
